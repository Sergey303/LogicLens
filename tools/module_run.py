...
def issue_run(
    *,
    ...
    track = next((item for item in world["manifest"]["tracks"] if item["id"] == track_id), None)
    ...
    with tempfile.TemporaryDirectory(prefix="logiclens-module-capsules-") as temporary:
        temp = Path(temporary)
        for dependency in module["usesCapsules"]:
            package = compile_capsule(world, dependency["id"], temp / dependency["id"])
    ...
    public_scenario["primaryOutcomes"] = role_variant.get("primaryOutcomes", [])
    public_scenario["mustEscalate"] = role_variant.get("mustEscalate", [])
    ...
    record: dict[str, Any] = {
        ...
        "capsules": sorted(capsule_packages, key=lambda item: item["id"]),
        ...
        "requiredOutputs": [
            ...
            if isinstance(stage, dict) and isinstance(stage.get("output"), str)
        ],
        "sourceHashes": source_hashes,
    }
    record["runHash"] = run_hash(record)
    run_schema = json_object(contracts_root / "learning-run-v0.schema.json", "learning run schema")
    ...
    (destination / "evaluator-frame.json").write_bytes(canonical_json(evaluator))
    (destination / "briefing.md").write_text(
        render_briefing(record, entry), encoding=UTF8, newline="\n"
    )
    (destination / "response-template.md").write_text(
        response_template(record), encoding=UTF8, newline="\n"
    )
    write_manifest(destination)
    return record


def verify_run(run_root: Path, contracts_root: Path) -> dict[str, Any]:
    root = run_root.resolve()
    manifest = json_object(root / "run-files.json", "run files manifest")
    ...
    evaluator = json_object(root / "evaluator-frame.json", "evaluator frame")
    if evaluator.get("runId") != record.get("runId") or evaluator.get("runHash") != supplied:
        raise ModuleRunError("evaluator frame is not bound to the learning run")
    return record
...
def write_manifest(destination: Path) -> None:
    ...
    (destination / "run-files.json").write_bytes(
        canonical_json({"schemaVersion": "0.1", "files": files})
    )


def render_briefing(record: dict[str, Any], entry: str) -> str:
    facts = record["scenario"].get("sharedFacts", {})
    fact_lines = "\n".join(f"- `{key}`: `{value}`" for key, value in facts.items())
    outcomes = "\n".join(f"- `{value}`" for value in record["scenario"].get("primaryOutcomes", []))
    escalations = "\n".join(f"- {value}" for value in record["scenario"].get("mustEscalate", []))
    return (
...
def run_hash(record: dict[str, Any]) -> str:
    payload = dict(record)
    payload.pop("runHash", None)
    digest = hashlib.sha256(RUN_DOMAIN + bytes((1,)) + canonical_json(payload))
...
if __name__ == "__main__":
    ...
    except (CapsuleError, ModuleRunError, OSError, ValueError) as exc:
