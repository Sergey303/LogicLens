...
def write_minimal_module(world: Path) -> None:
    ...
    write_json(
        module / "module.json",
        {
            ...
            "version": "0.1.0",
            "worldId": "management",
            "title": "PDF proposal fixture module",
            "usesCapsules": [
...
def build_world(root: Path) -> Path:
    ...
    write_json(
        world / "semantic" / "roles.json",
        {
            "schemaVersion": "0.1",
            "roles": [
                {"id": "role.product_owner", "title": "Product Owner"}
            ],
        },
    )
    write_json(
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="logiclens-pdf-test-") as temp_name:
        ...
        ir["irHash"] = domain_hash(
            ...
            {key: value for key, value in ir.items() if key != "irHash"},
        )
        ...
        record = {
            "schemaVersion": "0.1",
            "proposalId": "scrum-guide-2020-v0",
            "worldId": "management",
            "capsuleId": "management.role-boundaries",
            ...
            "pdf": {
                ...
                "finalUrl": "https://example.com/scrum-guide.pdf",
            },
            "processor": ir["processor"],
            "documentIr": {
                "path": "document/canonical-document-ir.json",
                "hash": sha256(document_path.read_bytes()),
                "pageCount": 1,
                "blockCount": len(ir["pages"][0]["blocks"]),
            },
        }
        ...
        fragments_path.write_bytes(
            b"".join(canonical_json(item) for item in fragments)
        )
        ...
        workspace = {
            ...
            "artifacts": {
                "snapshot": {
                    ...
                    "documentIrHash": sha256(document_path.read_bytes()),
                },
                "fragments": {
                    "path": "fragments/fragments.jsonl",
                    "count": len(fragments),
                    "hash": sha256(fragments_path.read_bytes()),
                },
            },
        }
            ...
            "assertions": [
                {
                    ...
                    "reviewNote": (
                        ...
                        "accountability for product value."
                    ),
                }
            ],
            "abstentions": [],
