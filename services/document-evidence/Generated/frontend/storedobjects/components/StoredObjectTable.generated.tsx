...
export interface StoredObjectTableOption {
  readonly label: string;
  readonly value: string;
}

export interface StoredObjectTableSort {
  readonly field: string;
  readonly direction: "asc" | "desc";
}

export interface StoredObjectTableFilter {
  readonly field: string;
...
export interface StoredObjectTableProps {
  readonly items: readonly StoredObjectDto[];
  ...
  readonly options?: Readonly<Record<string, readonly StoredObjectTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly StoredObjectTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly StoredObjectTableFilter[]) => void;
  readonly onOpen?: (item: StoredObjectDto) => void;
  readonly onEdit?: (item: StoredObjectDto) => void;
  readonly onDelete?: (item: StoredObjectDto) => void;
}

export function StoredObjectTable(props: StoredObjectTableProps) {
  ...
  return (
    ...
    >
      <Column
        ...
        filterElement={(options) => renderTextFilter("mediaType", options, props)}
      />
      <Column
        field="sha256"
        header="Sha256"
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("sha256", options, props)}
      />
      <Column
        field="sizeBytes"
        header="Size Bytes"
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("storageKey", options, props)}
      />
      <Column
        header="Actions"
        body={(row: StoredObjectDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function readSortEvent(event: unknown): readonly StoredObjectTableSort[] {
...
function readFilterEvent(event: unknown): readonly StoredObjectTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: StoredObjectTableFilter[] = [];
...
function renderActions(row: StoredObjectDto, props: StoredObjectTableProps) {
  return (
