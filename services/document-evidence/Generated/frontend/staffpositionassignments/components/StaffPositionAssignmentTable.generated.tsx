...
export interface StaffPositionAssignmentTableOption {
  readonly label: string;
  readonly value: string;
}

export interface StaffPositionAssignmentTableSort {
  readonly field: string;
  readonly direction: "asc" | "desc";
}

export interface StaffPositionAssignmentTableFilter {
  readonly field: string;
...
export interface StaffPositionAssignmentTableProps {
  readonly items: readonly StaffPositionAssignmentDto[];
  ...
  readonly options?: Readonly<Record<string, readonly StaffPositionAssignmentTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly StaffPositionAssignmentTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly StaffPositionAssignmentTableFilter[]) => void;
  readonly onOpen?: (item: StaffPositionAssignmentDto) => void;
  readonly onEdit?: (item: StaffPositionAssignmentDto) => void;
  readonly onDelete?: (item: StaffPositionAssignmentDto) => void;
}

export function StaffPositionAssignmentTable(props: StaffPositionAssignmentTableProps) {
  ...
  return (
    ...
    >
      <Column
        ...
        filterElement={(options) => renderTextFilter("assignmentKind", options, props)}
      />
      <Column
        field="endsAt"
        header="Ends At"
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("reason", options, props)}
      />
      <Column
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("userId", options, props)}
      />
      <Column
        header="Actions"
        body={(row: StaffPositionAssignmentDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function renderTextFilter(
  field: string,
  options: unknown,
  props: StaffPositionAssignmentTableProps,
) {
...
function readSortEvent(event: unknown): readonly StaffPositionAssignmentTableSort[] {
...
function readFilterEvent(event: unknown): readonly StaffPositionAssignmentTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: StaffPositionAssignmentTableFilter[] = [];
...
function renderActions(row: StaffPositionAssignmentDto, props: StaffPositionAssignmentTableProps) {
  return (
