...
export interface StaffPositionTableOption {
  readonly label: string;
  readonly value: string;
}

export interface StaffPositionTableSort {
  readonly field: string;
  readonly direction: "asc" | "desc";
}

export interface StaffPositionTableFilter {
  readonly field: string;
...
export interface StaffPositionTableProps {
  readonly items: readonly StaffPositionDto[];
  ...
  readonly options?: Readonly<Record<string, readonly StaffPositionTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly StaffPositionTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly StaffPositionTableFilter[]) => void;
  readonly onOpen?: (item: StaffPositionDto) => void;
  readonly onEdit?: (item: StaffPositionDto) => void;
  readonly onDelete?: (item: StaffPositionDto) => void;
}

export function StaffPositionTable(props: StaffPositionTableProps) {
  ...
  return (
    ...
    >
      <Column
        ...
        filterElement={(options) => renderTextFilter("code", options, props)}
      />
      <Column
        field="description"
        header="Description"
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("description", options, props)}
      />
      <Column
        field="isActive"
        header="Is Active"
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("name", options, props)}
      />
      <Column
        field="parentPositionId"
        header="Parent Position Id"
        sortable
        filter
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("parentPositionId", options, props)}
      />
      <Column
        header="Actions"
        body={(row: StaffPositionDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function readSortEvent(event: unknown): readonly StaffPositionTableSort[] {
...
function readFilterEvent(event: unknown): readonly StaffPositionTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: StaffPositionTableFilter[] = [];
...
function renderActions(row: StaffPositionDto, props: StaffPositionTableProps) {
  return (
