...
export interface RoleTableOption {
...
export interface RoleTableFilter {
  readonly field: string;
...
export interface RoleTableProps {
  ...
  readonly options?: Readonly<Record<string, readonly RoleTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly RoleTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly RoleTableFilter[]) => void;
  readonly onOpen?: (item: RoleDto) => void;
  readonly onEdit?: (item: RoleDto) => void;
  readonly onDelete?: (item: RoleDto) => void;
}
...
export function RoleTable(props: RoleTableProps) {
  ...
  return (
    ...
    >
      ...
      <Column
        header="Actions"
        body={(row: RoleDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function readSortEvent(event: unknown): readonly RoleTableSort[] {
...
function readFilterEvent(event: unknown): readonly RoleTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: RoleTableFilter[] = [];
...
function renderActions(row: RoleDto, props: RoleTableProps) {
  return (
