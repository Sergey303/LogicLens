...
export interface DocumentTableOption {
...
export interface DocumentTableFilter {
  readonly field: string;
...
export interface DocumentTableProps {
  ...
  readonly options?: Readonly<Record<string, readonly DocumentTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly DocumentTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly DocumentTableFilter[]) => void;
  readonly onOpen?: (item: DocumentDto) => void;
  readonly onEdit?: (item: DocumentDto) => void;
  readonly onDelete?: (item: DocumentDto) => void;
}
  ...
  return (
    ...
    >
      <Column
        ...
        header="Current Revision Number"
        sortable
        filter
      ...
      <Column
        ...
        filterElement={(options) => renderTextFilter("displayName", options, props)}
      />
      <Column
        field="isRevoked"
        header="Is Revoked"
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("sourceKind", options, props)}
      />
      <Column
        field="state"
        header="State"
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("workspaceId", options, props)}
      />
      <Column
        header="Actions"
        body={(row: DocumentDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function readSortEvent(event: unknown): readonly DocumentTableSort[] {
...
function readFilterEvent(event: unknown): readonly DocumentTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: DocumentTableFilter[] = [];
...
function renderActions(row: DocumentDto, props: DocumentTableProps) {
  return (
