...
export interface ProcessingJobTableOption {
  readonly label: string;
  readonly value: string;
}

export interface ProcessingJobTableSort {
...
export interface ProcessingJobTableProps {
  readonly items: readonly ProcessingJobDto[];
  ...
  readonly options?: Readonly<Record<string, readonly ProcessingJobTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly ProcessingJobTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly ProcessingJobTableFilter[]) => void;
  readonly onOpen?: (item: ProcessingJobDto) => void;
  readonly onEdit?: (item: ProcessingJobDto) => void;
  readonly onDelete?: (item: ProcessingJobDto) => void;
}

export function ProcessingJobTable(props: ProcessingJobTableProps) {
  ...
  return (
    ...
    >
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("idempotencyKey", options, props)}
      />
      <Column
        field="kind"
        header="Kind"
      ...
      <Column
        field="lastErrorCode"
        header="Last Error Code"
        sortable
        filter
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("lastErrorCode", options, props)}
      />
      <Column
        field="leaseUntil"
        header="Lease Until"
      ...
      <Column
        header="Actions"
        body={(row: ProcessingJobDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function readSortEvent(event: unknown): readonly ProcessingJobTableSort[] {
...
function readFilterEvent(event: unknown): readonly ProcessingJobTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: ProcessingJobTableFilter[] = [];
...
function renderActions(row: ProcessingJobDto, props: ProcessingJobTableProps) {
  return (
