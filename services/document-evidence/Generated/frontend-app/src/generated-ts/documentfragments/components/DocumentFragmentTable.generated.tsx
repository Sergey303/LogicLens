...
export interface DocumentFragmentTableOption {
  readonly label: string;
  readonly value: string;
}

export interface DocumentFragmentTableSort {
  readonly field: string;
  readonly direction: "asc" | "desc";
}

export interface DocumentFragmentTableFilter {
  readonly field: string;
...
export interface DocumentFragmentTableProps {
  readonly items: readonly DocumentFragmentDto[];
  ...
  readonly options?: Readonly<Record<string, readonly DocumentFragmentTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly DocumentFragmentTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly DocumentFragmentTableFilter[]) => void;
  readonly onOpen?: (item: DocumentFragmentDto) => void;
  readonly onEdit?: (item: DocumentFragmentDto) => void;
  readonly onDelete?: (item: DocumentFragmentDto) => void;
}

export function DocumentFragmentTable(props: DocumentFragmentTableProps) {
  ...
  return (
    ...
    >
      <Column
        ...
        filterElement={(options) => renderTextFilter("anchorJson", options, props)}
      />
      <Column
        field="contentHash"
        header="Content Hash"
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("contentHash", options, props)}
      />
      <Column
        field="documentRevisionId"
        header="Document Revision Id"
        sortable
        filter
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("documentRevisionId", options, props)}
      />
      <Column
        field="kind"
        header="Kind"
        ...
        filter
        filterMatchMode="in"
        filterElement={(options) => renderTagsFilter("kind", options, props)}
      />
      <Column
        field="sequence"
        header="Sequence"
      ...
      <Column
        ...
        filterMatchMode="contains"
        filterMatchModeOptions={textFilterMatchModes}
        filterElement={(options) => renderTextFilter("text", options, props)}
      />
      <Column
        header="Actions"
        body={(row: DocumentFragmentDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function renderTextFilter(
  field: string,
  options: unknown,
  props: DocumentFragmentTableProps,
) {
...
function readSortEvent(event: unknown): readonly DocumentFragmentTableSort[] {
...
function readFilterEvent(event: unknown): readonly DocumentFragmentTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: DocumentFragmentTableFilter[] = [];
...
function renderActions(row: DocumentFragmentDto, props: DocumentFragmentTableProps) {
  return (
