...
export interface PermissionTableSort {
  readonly field: string;
  readonly direction: "asc" | "desc";
}

export interface PermissionTableFilter {
  readonly field: string;
...
export interface PermissionTableProps {
  ...
  readonly options?: Readonly<Record<string, readonly PermissionTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly PermissionTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly PermissionTableFilter[]) => void;
  readonly onOpen?: (item: PermissionDto) => void;
  readonly onEdit?: (item: PermissionDto) => void;
  readonly onDelete?: (item: PermissionDto) => void;
}
...
export function PermissionTable(props: PermissionTableProps) {
  ...
  return (
    ...
    >
      ...
      <Column
        header="Actions"
        body={(row: PermissionDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function readSortEvent(event: unknown): readonly PermissionTableSort[] {
...
function readFilterEvent(event: unknown): readonly PermissionTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: PermissionTableFilter[] = [];
...
function renderActions(row: PermissionDto, props: PermissionTableProps) {
  return (
