...
export interface RolePermissionTableOption {
  readonly label: string;
  readonly value: string;
}

export interface RolePermissionTableSort {
  readonly field: string;
  readonly direction: "asc" | "desc";
}

export interface RolePermissionTableFilter {
  readonly field: string;
...
export interface RolePermissionTableProps {
  readonly items: readonly RolePermissionDto[];
  ...
  readonly options?: Readonly<Record<string, readonly RolePermissionTableOption[]>>;
  ...
  readonly onSortChange?: (sort: readonly RolePermissionTableSort[]) => void;
  readonly onFilterChange?: (filters: readonly RolePermissionTableFilter[]) => void;
  readonly onOpen?: (item: RolePermissionDto) => void;
  readonly onEdit?: (item: RolePermissionDto) => void;
  readonly onDelete?: (item: RolePermissionDto) => void;
}

export function RolePermissionTable(props: RolePermissionTableProps) {
  ...
  return (
    ...
    >
      <Column
        ...
        filterElement={(options) => renderTextFilter("permissionId", options, props)}
      />
      <Column
        field="roleId"
        header="Role Id"
      ...
      <Column
        header="Actions"
        body={(row: RolePermissionDto) => renderActions(row, props)}
        exportable={false}
      />
    </DataTable>
  );
...
function renderTextFilter(
  field: string,
  options: unknown,
  props: RolePermissionTableProps,
) {
...
function readSortEvent(event: unknown): readonly RolePermissionTableSort[] {
...
function readFilterEvent(event: unknown): readonly RolePermissionTableFilter[] {
  const source = (event as { readonly filters?: Record<string, unknown> }).filters ?? {};
  const filters: RolePermissionTableFilter[] = [];
...
function renderActions(row: RolePermissionDto, props: RolePermissionTableProps) {
  return (
