...
[ApiController]
[RestCrudController("staff.position.role")] 
[Route("api/staffpositionroles")]
public sealed class StaffPositionRoleController : ControllerBase
{
    private readonly StaffPositionRoleService _service;

    public StaffPositionRoleController(StaffPositionRoleService service)
    {
        _service = service;
    }

    [PolicyGate("StaffPositionRole.Read")]
    [HttpGet]
    public async Task<ActionResult<ListStaffPositionRoleResult>> List(
        [FromQuery] ListStaffPositionRoleRequest request,
    ...
    [PolicyGate("StaffPositionRole.Read")]
    [HttpGet("{id}")]
    public async Task<ActionResult<StaffPositionRoleDto>> Get(
        Guid id,
        CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(typeof(IReadOnlyList<StaffPositionRoleLookupDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<StaffPositionRoleLookupDto>>> Lookup(
        [FromQuery] LookupStaffPositionRoleRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<StaffPositionRoleSuggestionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<StaffPositionRoleSuggestionDto>>> Suggest(
        string field,
        [FromQuery] SuggestStaffPositionRoleRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<StaffPositionRoleOptionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<StaffPositionRoleOptionDto>>> Options(
        string field,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
    ...
    [PolicyGate("StaffPositionRole.Create")]
    [HttpPost]
    public async Task<ActionResult<StaffPositionRoleDto>> Create(
    ...
    [PolicyGate("StaffPositionRole.Update")]
    [HttpPut("{id}")]
    public async Task<ActionResult<StaffPositionRoleDto>> Update(
        Guid id,
    ...
    [PolicyGate("StaffPositionRole.Delete")]
    [HttpDelete("{id}")]
    ...
    {
        return HasPermission("StaffPositionRole.Read")
            || CanReadAll()
            || CanReadOwn();
    }
    ...
    {
        return HasPermission("StaffPositionRole.Create")
            || CanCreateAll()
            || CanCreateOwn();
    }
    ...
    {
        return HasPermission("StaffPositionRole.Update")
            || CanUpdateAll()
            || CanUpdateOwn();
    }
    ...
    {
        return HasPermission("StaffPositionRole.Delete")
            || CanDeleteAll()
            || CanDeleteOwn();
    }
    ...
    private IQueryable<StaffPositionRole> ApplyReadRowAuthorization(IQueryable<StaffPositionRole> query)
    {
        if (CanReadAll())
        {
            return query;
    ...
    private bool CanAccessRead(StaffPositionRole entity)
    {
        if (CanReadAll())
        {
            return true;
    ...
    private bool CanAccessUpdate(StaffPositionRole entity)
    {
        if (CanUpdateAll())
        {
            return true;
    ...
    private bool CanAccessDelete(StaffPositionRole entity)
    {
        if (CanDeleteAll())
        {
            return true;
