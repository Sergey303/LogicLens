...
[ApiController]
[RestCrudController("document.revision")] 
[Route("api/documentrevisions")]
public sealed class DocumentRevisionController : ControllerBase
{
    private readonly DocumentRevisionService _service;
    ...
    [PolicyGate("DocumentRevision.Read")]
    [HttpGet]
    public async Task<ActionResult<ListDocumentRevisionResult>> List(
        [FromQuery] ListDocumentRevisionRequest request,
    ...
    [PolicyGate("DocumentRevision.Read")]
    [HttpGet("{id}")]
    public async Task<ActionResult<DocumentRevisionDto>> Get(
        Guid id,
        CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(typeof(IReadOnlyList<DocumentRevisionLookupDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentRevisionLookupDto>>> Lookup(
        [FromQuery] LookupDocumentRevisionRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<DocumentRevisionSuggestionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentRevisionSuggestionDto>>> Suggest(
        string field,
        [FromQuery] SuggestDocumentRevisionRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<DocumentRevisionOptionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentRevisionOptionDto>>> Options(
        string field,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
    ...
    [PolicyGate("DocumentRevision.Create")]
    [HttpPost]
    public async Task<ActionResult<DocumentRevisionDto>> Create(
    ...
    [PolicyGate("DocumentRevision.Update")]
    [HttpPut("{id}")]
    public async Task<ActionResult<DocumentRevisionDto>> Update(
        Guid id,
    ...
    [PolicyGate("DocumentRevision.Delete")]
    [HttpDelete("{id}")]
    ...
    {
        return HasPermission("DocumentRevision.Read")
            || CanReadAll()
            || CanReadOwn();
    }
    ...
    {
        return HasPermission("DocumentRevision.Create")
            || CanCreateAll()
            || CanCreateOwn();
    }
    ...
    {
        return HasPermission("DocumentRevision.Update")
            || CanUpdateAll()
            || CanUpdateOwn();
    }
    ...
    {
        return HasPermission("DocumentRevision.Delete")
            || CanDeleteAll()
            || CanDeleteOwn();
    }
    ...
    private IQueryable<DocumentRevision> ApplyReadRowAuthorization(IQueryable<DocumentRevision> query)
    {
        if (CanReadAll())
        {
            return query;
    ...
    private bool CanAccessRead(DocumentRevision entity)
    {
        if (CanReadAll())
        {
            return true;
    ...
    private bool CanAccessUpdate(DocumentRevision entity)
    {
        if (CanUpdateAll())
        {
            return true;
    ...
    private bool CanAccessDelete(DocumentRevision entity)
    {
        if (CanDeleteAll())
        {
            return true;
