...
[ApiController]
[RestCrudController("staff.position")] 
[Route("api/staffpositions")]
public sealed class StaffPositionController : ControllerBase
{
    private readonly StaffPositionService _service;

    public StaffPositionController(StaffPositionService service)
    {
        _service = service;
    }

    [PolicyGate("StaffPosition.Read")]
    [HttpGet]
    public async Task<ActionResult<ListStaffPositionResult>> List(
        [FromQuery] ListStaffPositionRequest request,
    ...
    [PolicyGate("StaffPosition.Read")]
    [HttpGet("{id}")]
    public async Task<ActionResult<StaffPositionDto>> Get(
        Guid id,
        CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(typeof(IReadOnlyList<StaffPositionLookupDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<StaffPositionLookupDto>>> Lookup(
        [FromQuery] LookupStaffPositionRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<StaffPositionSuggestionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<StaffPositionSuggestionDto>>> Suggest(
        string field,
        [FromQuery] SuggestStaffPositionRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<StaffPositionOptionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<StaffPositionOptionDto>>> Options(
        string field,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
    ...
    [PolicyGate("StaffPosition.Create")]
    [HttpPost]
    public async Task<ActionResult<StaffPositionDto>> Create(
    ...
    [PolicyGate("StaffPosition.Update")]
    [HttpPut("{id}")]
    public async Task<ActionResult<StaffPositionDto>> Update(
        Guid id,
    ...
    [PolicyGate("StaffPosition.Delete")]
    [HttpDelete("{id}")]
    ...
    {
        return HasPermission("StaffPosition.Read")
            || CanReadAll()
            || CanReadOwn();
    }
    ...
    {
        return HasPermission("StaffPosition.Create")
            || CanCreateAll()
            || CanCreateOwn();
    }
    ...
    {
        return HasPermission("StaffPosition.Update")
            || CanUpdateAll()
            || CanUpdateOwn();
    }
    ...
    {
        return HasPermission("StaffPosition.Delete")
            || CanDeleteAll()
            || CanDeleteOwn();
    }
    ...
    private IQueryable<StaffPosition> ApplyReadRowAuthorization(IQueryable<StaffPosition> query)
    {
        if (CanReadAll())
        {
            return query;
    ...
    private bool CanAccessRead(StaffPosition entity)
    {
        if (CanReadAll())
        {
            return true;
    ...
    private bool CanAccessUpdate(StaffPosition entity)
    {
        if (CanUpdateAll())
        {
            return true;
    ...
    private bool CanAccessDelete(StaffPosition entity)
    {
        if (CanDeleteAll())
        {
            return true;
