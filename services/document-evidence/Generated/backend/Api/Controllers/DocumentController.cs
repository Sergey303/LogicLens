...
public sealed class DocumentController : ControllerBase
{
    ...
    [PolicyGate("Document.Read")]
    [HttpGet]
    public async Task<ActionResult<ListDocumentResult>> List(
    ...
    [ProducesResponseType(typeof(IReadOnlyList<DocumentLookupDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentLookupDto>>> Lookup(
        [FromQuery] LookupDocumentRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<DocumentSuggestionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentSuggestionDto>>> Suggest(
        string field,
        [FromQuery] SuggestDocumentRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<DocumentOptionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentOptionDto>>> Options(
        string field,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
    ...
    private IQueryable<Document> ApplyReadRowAuthorization(IQueryable<Document> query)
    {
        if (CanReadAll())
        {
            return query;
    ...
    private bool CanAccessRead(Document entity)
    {
        if (CanReadAll())
        {
            return true;
    ...
    private bool CanAccessUpdate(Document entity)
    {
        if (CanUpdateAll())
        {
            return true;
    ...
    private bool CanAccessDelete(Document entity)
    {
        if (CanDeleteAll())
        {
            return true;
