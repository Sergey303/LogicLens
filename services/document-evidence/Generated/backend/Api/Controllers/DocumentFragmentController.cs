...
[ApiController]
[RestCrudController("document.fragment")] 
[Route("api/documentfragments")]
public sealed class DocumentFragmentController : ControllerBase
{
    private readonly DocumentFragmentService _service;

    public DocumentFragmentController(DocumentFragmentService service)
    {
        _service = service;
    }

    [PolicyGate("DocumentFragment.Read")]
    [HttpGet]
    public async Task<ActionResult<ListDocumentFragmentResult>> List(
        [FromQuery] ListDocumentFragmentRequest request,
    ...
    [PolicyGate("DocumentFragment.Read")]
    [HttpGet("{id}")]
    public async Task<ActionResult<DocumentFragmentDto>> Get(
        Guid id,
        CancellationToken cancellationToken)
    {
    ...
    [ProducesResponseType(typeof(IReadOnlyList<DocumentFragmentLookupDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentFragmentLookupDto>>> Lookup(
        [FromQuery] LookupDocumentFragmentRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<DocumentFragmentSuggestionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentFragmentSuggestionDto>>> Suggest(
        string field,
        [FromQuery] SuggestDocumentFragmentRequest request,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
        {
    ...
    [ApiOperation(ApiOperationKind.Query)]
    [ProducesResponseType(typeof(IReadOnlyList<DocumentFragmentOptionDto>), 200)]
    public async Task<ActionResult<IReadOnlyList<DocumentFragmentOptionDto>>> Options(
        string field,
        CancellationToken cancellationToken)
    {
        if (!CanRead())
    ...
    [PolicyGate("DocumentFragment.Create")]
    [HttpPost]
    public async Task<ActionResult<DocumentFragmentDto>> Create(
    ...
    [PolicyGate("DocumentFragment.Update")]
    [HttpPut("{id}")]
    public async Task<ActionResult<DocumentFragmentDto>> Update(
        Guid id,
    ...
    [PolicyGate("DocumentFragment.Delete")]
    [HttpDelete("{id}")]
    ...
    {
        return HasPermission("DocumentFragment.Read")
            || CanReadAll()
            || CanReadOwn();
    }
    ...
    {
        return HasPermission("DocumentFragment.Create")
            || CanCreateAll()
            || CanCreateOwn();
    }
    ...
    {
        return HasPermission("DocumentFragment.Update")
            || CanUpdateAll()
            || CanUpdateOwn();
    }
    ...
    {
        return HasPermission("DocumentFragment.Delete")
            || CanDeleteAll()
            || CanDeleteOwn();
    }
    ...
    private IQueryable<DocumentFragment> ApplyReadRowAuthorization(IQueryable<DocumentFragment> query)
    {
        if (CanReadAll())
        {
            return query;
    ...
    private bool CanAccessRead(DocumentFragment entity)
    {
        if (CanReadAll())
        {
            return true;
    ...
    private bool CanAccessUpdate(DocumentFragment entity)
    {
        if (CanUpdateAll())
        {
            return true;
    ...
    private bool CanAccessDelete(DocumentFragment entity)
    {
        if (CanDeleteAll())
        {
            return true;
