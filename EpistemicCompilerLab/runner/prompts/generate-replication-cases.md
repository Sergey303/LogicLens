...
- `success`: revision A with any listed date, or revision B with a listed date;
- `unknown`: revision C with a listed date;
- `missing_date`: revision A or B is explicit, but no calendar date appears;
...
- distribute non-missing dates across `words`, `dotted` and `iso` styles;
...
- in about half the cases add a harmless distractor phrase about a board, installation, inspection, batch or documentation, without adding another date or revision;
