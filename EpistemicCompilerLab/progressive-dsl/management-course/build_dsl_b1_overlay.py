#!/usr/bin/env python3
"""Build a temporary non-guessable DSL-B1 management overlay."""
...
UTF8 = "utf-8"
OVERLAY_VERSION = "0.2.1"
RULE_RELATIVE = "rules/logical-rules-dsl-b1-v0.jsonl"
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--rules",
        type=Path,
        default=Path(__file__).with_name("dsl-b1-logical-rules-v0.jsonl"),
    )
    parser.add_argument(
        "--contracts-root",
        type=Path,
...
def main() -> int:
    ...
    if any(
        ...
        raise RuntimeError("DSL-B1 rule overlay is already declared")
    rule_files.append(
        {
            "path": RULE_RELATIVE,
            "kind": "rules",
            "description": (
                "Experimental non-guessable local policy rules for DSL-B1: "
                "multi-hop, any, notExplicit, explicit refutation and conflict."
            ),
        }
    )
    capsule["version"] = OVERLAY_VERSION
    capsule["description"] = (
        "Experimental DSL-B1 overlay for non-guessable management-policy cases. "
        "The source knowledge remains unchanged."
    )
    ...
    overlay = {
        "schemaVersion": "0.1",
        "overlayId": "management-progressive-dsl-b1-v0",
        "dslLevel": "DSL-B",
        "benchmarkTranche": "B1-non-guessable-policy",
    ...
    write_json(output / "dsl-b1-overlay.json", overlay)
    print("Built and validated management DSL-B1 overlay")
